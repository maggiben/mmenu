...
#define VERSION 0.8.0		// 21/03/2008
...
HMENU menu_builder(HWND parent)
{
	...
	if(!menu_is_done)
	{
		...
		dhUninitialize(TRUE);
		menu_is_done = TRUE;
		xmlInitnstance("cd_catalog2.xml",FALSE);
	}
	return main_menu;
...
LRESULT CALLBACK menuproc_sclass(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	*/
	...
	case WM_WINDOWPOSCHANGING:
	{
		...
		{
			lpwinpos->x += 5;	//Move the sub-menu's window right
...
HRESULT AnsiToUnicode(LPCSTR pszA, LPCOLESTR *ppszW)
{
    ...
    *ppszW = (LPCOLESTR) CoTaskMemAlloc(length * 2);
    if (NULL == *ppszW) return E_OUTOFMEMORY;
