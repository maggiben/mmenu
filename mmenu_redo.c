...
//  * Rewrite the Sidebar code (needed to be an independent item                 //
...
static int 			parseXML						(IDispatch*,HMENU);
...
///////////////////////////////////////////////////////////////////////////////////
static int parseXML(IDispatch *NodeList, HMENU holder)
{
...
LRESULT CALLBACK menuproc_sclass(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	*/
	case WM_NCPAINT:
	{
		...
		//Paint the black rectangle around the sidebar
...
int draw_menu(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
		...
		if(menu_data->menu_type == MF_SIDEBAR)
		{
			...
			rct.right = rct.right - 5;
			HBRUSH hBrush = CreateSolidBrush(RGB(128, 128, 138));
			...
			COLORREF prevColor = SetTextColor(lpDs->hDC, RGB(90,90,90)); //190
			HFONT hfntPrev = SelectObject(lpDs->hDC, hfnt);

			BOOL ok = TextOut(lpDs->hDC, rct.left + ((SIDEBAR_WIDTH - 15) / 2) , rct.bottom - 10, lpszLable, lstrlen(lpszLable));		//Restore defaults
			SelectObject(lpDs->hDC, hfntPrev);
			SetTextColor(lpDs->hDC, prevColor);
			
			prevColor = SetTextColor(lpDs->hDC, RGB(210,210,210)); //190
			hfntPrev = SelectObject(lpDs->hDC, hfnt);

			ok = TextOut(lpDs->hDC, rct.left + ((SIDEBAR_WIDTH - 14) / 2) , rct.bottom - 9, lpszLable, lstrlen(lpszLable));		//Restore defaults
			SelectObject(lpDs->hDC, hfntPrev);
			SetTextColor(lpDs->hDC, prevColor);
