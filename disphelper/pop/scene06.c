...
GLfloat e_FogColor[]={ 1.0f, 1.0f, 1.0f, 1.0f };

GLfloat	e_xrot;
GLfloat	e_yrot;
GLfloat	e_zrot;
GLfloat e_timer=-1.25f;
GLfloat e_speed=0.0f;

GLfloat e_zeta=0.0f;
GLfloat e_fade=0.0f;

GLfloat e_rocca=1.0f;
GLfloat e_depth=60.0f;

GLfloat e_radius=0.0f;
GLuint shiplist;
...
void  e_InitGL(void)
{
	...
	Texture_Load(&e_Text[1], "data/star.raw");
	Texture_Load(&e_Text[2], "data/cl.raw");
	Texture_Load(&e_Text[3], "data/mtop.raw");
	Texture_Load(&e_Text[4], "data/text2.raw");
	Texture_Load(&e_Text[5], "data/metal.raw");
	Texture_Load(&e_Text[6], "data/rusty2.raw");
	Texture_Load(&e_Text[7], "data/mfloor1.raw");
	Texture_Load(&e_Text[8], "data/ship.raw");
	Texture_Load(&e_Text[9], "data/sground.raw");
...
bool e_DrawGLScene(GLfloat globtime)
{
	...
	else
	{
		...
		e_Clear(.5f-.5*cos((e_timer-eoffset)*3.1415));
	}
	...
	if (e_scene)
	{
		...
		if (e_timer>-1.0f)
		{
			...
			if ((e_timer>6.0f)&&(e_timer<10.0)) 
				e_xrot=(6.0f-((e_timer-6.0f)*1.5f))*(cos((e_timer-6.0f)*3.1415f/2.0f)-1.0f);
			
			if ((e_timer>5.0f)&&(e_timer<9.0)) 
				e_yrot=(36.0f-((e_timer-5.0f)*9.0f))*(cos((e_timer-5.0f)*3.1415f/2.0f)-1.0f);
			
			if ((e_timer>2.0f)&&(e_timer<6.0)) 
				e_zrot=-(9.0f-((e_timer-2.0f)*4.5f))*(cos((e_timer-2.0f)*3.1415f/2.0f)-1.0f);
			
			
			if ((e_timer>10.0f)&&(e_timer<12.0)) 
				e_xrot=-(6.0f-((e_timer-10.0f)*3.0f))*(cos((e_timer-10.0f)*3.1415f)-1.0f);
			
			if ((e_timer>12.0f)&&(e_timer<14.0)) 
				e_xrot=(6.0f-((e_timer-12.0f)*3.0f))*(cos((e_timer-12.0f)*3.1415f)-1.0f);
			
			if ((e_timer>9.0f)&&(e_timer<13.0)) 
				e_yrot=-(6.0f-((e_timer-9.0f)*1.5f))*(cos((e_timer-9.0f)*3.1415f/2.0f)-1.0f);
			
			if ((e_timer>6.0f)&&(e_timer<10.0)) 
				e_zrot=-(16.0f-((e_timer-6.0f)*4.0f))*(cos((e_timer-6.0f)*3.1415f/2.0f)-1.0f);
			
			if ((e_timer>10.0f)&&(e_timer<16.0)) 
				e_zrot=(6.0f-((e_timer-10.0f)*1.0f))*(cos((e_timer-10.0f)*3.1415f/3.0f)-1.0f);
			
			if ((e_timer>16.0f)&&(e_timer<20.0)) 
				e_zrot=(9.0f-((e_timer-16.0f)*4.5f))*(cos((e_timer-16.0f)*3.1415f/3.0f)-1.0f);
			
			if ((e_timer>13.5f)&&(e_timer<15.0f)) 
				e_radius=(cos((e_timer-13.5f)*3.1415f/1.5f+3.1415f)+1.0f)/2.0f;
			
			if ((e_timer>13.25f)&&(e_timer<19.25f)) 
				e_speed=.125+(cos(3.1415f+(e_timer-13.25f)*3.1415f/4.0f)+1.0f)/50.0f;//e_speed=.125+sin(3.1415f*.5f*(e_timer-13.25))*.0125f;//e_speed=.125+(cos(3.1415f+(e_timer-13.25f)*3.1415f/2.0f)+1.0f)/50.0f;
			
			if (e_timer>16.0f) 
				e_fade=(cos((e_timer-16.0f)*3.1415f/2.0f+3.1415f)+1.0f)*20.0f;
			...
			for(int fb=0; fb<lim; fb++)
			{
				...
				Texture_Use(&e_Text[3]);
				glScalef(1.122,1,e_depth);
				glTranslatef(0,2.591,0);
				glRotatef(90,1,0,0);
				if (e_timer<1.0f) 
					glColor4f(e_timer/2.0f,e_timer/2.0f,e_timer/2.0f,e_timer/2.0f);
				else 
			...
			if ((e_timer>1.0f)&&(e_timer<15.0f))
			{
				...
				for( int pp=0; pp<2; pp++)
				{
					...
					for (int zx=0; zx<21; zx++)			// LIGHTS SIDE
					{
						...
						glColor4f(1.0f,1.0f,1.0f,1.0f);
						if (e_timer>3) 
							e_drawtrail(.75+1.5*var);
						else if (e_timer>2.625) 
			...
			glEnable(GL_DEPTH_TEST);
			if (e_timer<0.0f) 
				e_radius=cos(e_timer*3.1415/2.0f-3.1415/2.0f)+1.0f;//+3.1415/2.0f)+1.0f;
			else
				if (e_timer<1.0f) 
					e_radius=cos(e_timer*3.1415/2.0f+3.1415/2.0f)+1.0f;
			...
			if (e_timer>16.5) 
				glFogf(GL_FOG_DENSITY,.075+(e_timer-16.5)*(e_timer-16.5)*3.0);
	...
	if (((e_timer>5)&&(e_timer<9.75))||((e_timer>12)&&(e_timer<14.1)))
	{
		...
		if (e_timer<7.7) 
			glTranslatef(-.25-.5*sin(e_timer),.5-.5*cos(e_timer),46-6*e_timer);
		else if (e_timer<8.7) 
			glTranslatef(-.25-.5*sin(e_timer),.5-.5*cos(e_timer),46-6*e_timer+4*(.5-.5*cos((e_timer-7.7)*3.1415)));
		else if (e_timer<11) 
			glTranslatef(-.25-.5*sin(e_timer)+(e_timer-8.7)*(e_timer-8.7),.5-.5*cos(e_timer)-5*(e_timer-8.7)*(e_timer-8.7),50-6*e_timer-250*(e_timer-8.7)*(e_timer-8.7)*(e_timer-8.7));
		else 
			glTranslatef(.5-.5*sin(e_timer),1-.25*(e_timer-12)*(e_timer-12),-4*(e_timer-12)*(e_timer-12)*(e_timer-12));
		
		glPushMatrix();
		if (e_timer<11) 
			glRotatef(60*sin(e_timer*2.5)*sin(e_timer*1.5),0,0,1);
		else 
			glRotatef(-70-120*sin((e_timer-11.7)*4.0),0,0,1);
		glDisable(GL_BLEND);
		glTexGeni(GL_T, GL_TEXTURE_GEN_MODE, GL_SPHERE_MAP);
		glEnable(GL_TEXTURE_GEN_S);
		glEnable(GL_TEXTURE_GEN_T);
		Texture_Use(&e_Text[8]);
		...
		if (e_timer>8.6)
		{
			...
			Texture_Use(&e_Text[9]);
			
			if (e_timer<13) 
				glColor4f(1,1,1,.5);
			else 
				glColor4f(1,1,1,.5-.5*(e_timer-13.0)*1.1);
	...
	if ((e_timer>1.0f)&&(e_timer<2.0f)) 
		e_speed=sin(3.1415f*.5f*(e_timer-1.0))*.125f;
