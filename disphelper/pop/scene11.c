...
GLfloat j_FogColor[]={1.0f,1.0f,1.0f,1.0f};
int j_x, j_y;
GLfloat j_max=0.0f;

GLfloat j_zeta=-6.7f;
GLfloat j_radius=0.0f;
...
void j_InitGL(void)
{
	glutSetWindowTitle("Scene 11");
	j_max=0.0f;
	Texture_Load(&j_Text[1], "data/provaz.raw");
	Texture_Load(&j_Text[2], "data/skygs.raw");
	Texture_Load(&j_Text[3], "data/white.raw");
	Texture_Load(&j_Text[4], "data/moon.raw");
	Texture_Load(&j_Text[5], "data/stars1.raw");
	Texture_Load(&j_Text[6], "data/moonmask.raw");
	Texture_Load(&j_Text[7], "data/spread.raw");
	Texture_Load(&j_Text[8], "data/circle.raw");
	Texture_Load(&j_Text[9], "data/circleempty.raw");
	Texture_Load(&j_Text[10], "data/circlefill.raw");
	Texture_Load(&j_Text[11], "data/noise.raw");
	Texture_Load(&j_Text[12], "data/tail.raw");
	
	glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	Texture_Load(&j_Text[13], "data/profile.raw");
	Texture_Load(&j_Text[14], "data/star.raw");
	Texture_Load(&j_Text[15], "data/land.raw");
	
	glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_S, GL_CLAMP);
	glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_T, GL_CLAMP);
	Texture_Load(&j_Text[16], "data/esaflr.raw");
	Texture_Load(&j_Text[17], "data/credits.raw");
	Texture_Load(&j_Text[18], "data/creditsneg.raw");
...
void j_drawtrail(UINT thickness, INT alpha, GLfloat length, UINT r,UINT g,UINT b,UINT a)
{
	glPushMatrix();
...
bool j_DrawGLScene(GLfloat globtime)
{
	...
	if (j_radius>.0f)
	{
		if (j_radius<1.5f)
		{
			...
			//&j_Text[6].Use();
			glColor4ub(255,255,255,148*(j_radius-.35));
			glBlendFunc(GL_SRC_ALPHA,GL_ONE);
			//glBindTexture(GL_TEXTURE_2D,&j_Text[ure[3]);
			Texture_Use(&j_Text[4]);
			...
			if ((j_radius-.5f<.053)&&(j_radius-.5f>-.007f))
			{
				...
				Texture_Use(&j_Text[12]);
				
				glBlendFunc(GL_SRC_ALPHA,GL_ONE);
				glRotatef(100,0,0,1);		// SCIA
				glScalef(.1,sin(55*(j_radius-.5f))*sin(55*(j_radius-.5f)),1);
				glColor4ub(255,255,255,64*sin(55*(j_radius-.5f))*sin(55*(j_radius-.5f)));
				j_drawquad(1);
			}
			...
			if ((j_radius<.08)&&(j_radius>.05))
			{
				...
				glColor4ub(255,255,255,255*sin(100*(-.05+j_radius))*sin(100*(-.05+j_radius)));
				j_drawquad(1);
			}
			...
			if (j_radius*3.0f<3.1415)
			{
				...
				Texture_Use(&j_Text[9]);
				
				glColor4ub(128,160,255,32*sin(j_radius*3.0f));
				...
				glColor4ub(192,48,16,80*sin(j_radius*3.0f));
				...
				Texture_Use(&j_Text[10]);
				
				glColor4ub(64,192,96,100*sin(j_radius*3.0f));
				glTranslatef(j_radius*1.81f,0,0);
				j_drawquad(.2);
				glColor4ub(255,255,255,100*sin(j_radius*3.0f));
				j_drawquad(.1);
				glPopMatrix();
				
				glPushMatrix();
				//BindTexture(GL_TEXTURE_2D, &j_Text[ure[7]);
				Texture_Use(&j_Text[8]);
				
				glColor4ub(96,128,192,64*sin(j_radius*3.0f));
				...
				glColor4ub(192,192,160,64*sin(j_radius*3.0f));
				...
				glColor4ub(132,160,148,32*sin(j_radius*3.0f));
		...
		Texture_Use(&j_Text[15]);
		
		glRotatef(90,0,0,1);
		glScalef(1.2,-3.4,1);
		if (j_radius<3.9f) 
			glColor4ub(192-192*j_radius/4,192-192*j_radius/4,192-192*j_radius/4,255);
		else 
			glColor4ub(192-192*3.9f/4,192-192*3.9f/4,192-192*3.9f/4,255);
		j_drawquad(1);
		glEnable(GL_BLEND);
		...
		if ((j_radius>.8f)&&(j_radius<1.115))
		{
			...
			glColor4ub(255,255,255,160*sin((j_radius-.8f)*10)*sin((j_radius-.8f)*10));
			j_drawquad(1.75);
		}
		...
		for (int i=0; i<j_num; i++)
		{
			...
			glColor4ub(parts[i].r,parts[i].g,parts[i].b,j_max*j_max*parts[i].a*(.75+.25*sin(parts[i].phase+j_radius*(i))));
		...
		for (int i=0; i<j_num/5; i++)
		{
			...
			{
				if (parts[i].xp>0.0f) 
					glColor4ub(128+127*parts[i].xp*4,128-128*parts[i].xp*4,128-128*parts[i].xp*4,alpha);
				else 
