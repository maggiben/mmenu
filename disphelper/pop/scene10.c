...
GLfloat i_timer=0.0f;
GLfloat i_radius[7];
GLfloat i_zeta=0.4f;
GLfloat i_scale;
GLfloat i_shade;
GLfloat i_incr=180.0f;
...
i_part rays[i_numray];
int i_alpha[i_num];

void i_InitGL(void)
{
	glutSetWindowTitle("Scene 10");
	Texture_Load(&i_Text[1], "data/you.raw");
	Texture_Load(&i_Text[2], "data/youglow.raw");
	Texture_Load(&i_Text[3], "data/gotta.raw");
	Texture_Load(&i_Text[4], "data/gottaglow.raw");
	Texture_Load(&i_Text[5], "data/say.raw");
	Texture_Load(&i_Text[6], "data/sayglow.raw");
	Texture_Load(&i_Text[7], "data/yes.raw");
	Texture_Load(&i_Text[8], "data/yesglow.raw");
	Texture_Load(&i_Text[9], "data/cl.raw");
	Texture_Load(&i_Text[10], "data/text1.raw");
	Texture_Load(&i_Text[11], "data/xp10.raw");
	Texture_Load(&i_Text[12], "data/basic2.raw");
	Texture_Load(&i_Text[13], "data/cl2.raw");
	Texture_Load(&i_Text[14], "data/excess.raw");
	Texture_Load(&i_Text[15], "data/excessglow.raw");
	Texture_Load(&i_Text[16], "data/another.raw");
	Texture_Load(&i_Text[17], "data/anotherglow.raw");
	Texture_Load(&i_Text[18], "data/to.raw");
	Texture_Load(&i_Text[19], "data/toglow.raw");
	Texture_Load(&i_Text[20], "data/esaflr.raw");
	...
	for (i=0;i<i_numray;i++)
		i_rst(i);
	
	for (i=0;i<i_num;i++)
		i_alpha[i]=128-12.8*(GLfloat)i/i_num;
...
bool i_DrawGLScene(GLfloat globtime)
{
	...
	glColor4f(cos(i_radius[0])*cos(i_radius[0])+cos(i_radius[3])*cos(i_radius[3])+cos(i_radius[4])*cos(i_radius[4])+cos(i_radius[5])*cos(i_radius[5]),
		...
		cos(i_radius[2])*cos(i_radius[2])+cos(i_radius[3])*cos(i_radius[3])+cos(i_radius[6])*cos(i_radius[6])+cos(i_radius[6])*cos(i_radius[6]),.75f);
	glPushMatrix();
	//glBindTexture(GL_TEXTURE_2D, texture[11]);
	Texture_Use(&i_Text[12]);
