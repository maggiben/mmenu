...
GLfloat g_points[64][64];
bool playjet=true;
#define G_NUM 50
#define G_NUM1 250

const int g_num=50;
const int g_num1=250;
long g_gettime=0;
GLfloat g_ext=10.0f;
typedef struct
{
...
GLfloat g_litetop=1.0f;
GLfloat g_liteleft=0.5f;
GLfloat g_literite=0.25f;

GLfloat g_rot=1.5f;
GLfloat g_rota=0.0;
GLfloat g_zeta=1;
int g_phase[64][64];
GLuint g_speed[64][64];
GLuint g_gx,g_gy;
GLuint g_a=0,g_b=0,g_c=1;
GLfloat g_radius=-6.0f;

GLfloat g_FogColor[]={ 1.0f, 1.0f, 1.0f, 1.0f };

void g_Clean(void)
{
	for(int i = 0; i<numtexs; i++)
	{
		Texture_Kill(&g_Text[i]);
	}
	init=true;
}
...
void g_InitGL(void)
{
	...
	Texture_Load(&g_Text[0], "data/esaflr.raw");
	Texture_Load(&g_Text[1], "data/senv36.raw");  // MOON
	Texture_Load(&g_Text[2], "data/sky7.raw");	 // MOON
	Texture_Load(&g_Text[3], "data/f16.raw");
	Texture_Load(&g_Text[4], "data/f16mask.raw");
	Texture_Load(&g_Text[5], "data/basic2.raw");
	Texture_Load(&g_Text[6], "data/sun2.raw");
	Texture_Load(&g_Text[7], "data/trail.raw");
	Texture_Load(&g_Text[8], "data/trailleft.raw");
	Texture_Load(&g_Text[9], "data/trailright.raw");
	Texture_Load(&g_Text[10], "data/floodmask.raw");
	Texture_Load(&g_Text[11], "data/env26.raw"); //DUSK
	Texture_Load(&g_Text[12], "data/sky27.raw"); //DUSK
	Texture_Load(&g_Text[13], "data/env17.raw"); //DAY
	Texture_Load(&g_Text[14], "data/sky17.raw"); //DAY
	Texture_Load(&g_Text[15], "data/basic2.raw");
	Texture_Load(&g_Text[16], "data/white.raw");
	Texture_Load(&g_Text[17], "data/moon1.raw");
	...
	for(x=0; x<64; x++)
	{
		for(int y=0; y<63; y++)
		{
			g_phase[x][y]=.001*(rand()%5000);
			g_speed[x][y]=.1+.001*(rand()%10000);
		}
	}
	for (x=0; x<64; x++)
	{
...
bool g_DrawGLScene(GLfloat globtime)
{
	...
	if (!g_scene)
		Texture_Use(&g_Text[1]);
	else if (g_scene==1)
		Texture_Use(&g_Text[11]);
	else
		Texture_Use(&g_Text[13]);
	...
	for( g_gx = 0; g_gx < 63; g_gx++ )
	{
		...
		{
			glVertex3f(g_gx/1.5f, (g_gy), g_points[g_gx][g_gy]);
			glVertex3f((g_gx+1)/1.5f, (g_gy), g_points[g_gx+1][g_gy]);
			glVertex3f((g_gx+1)/1.5f, (g_gy+1), g_points[g_gx+1][g_gy+1]);
			glVertex3f((g_gx)/1.5f, (g_gy+1), g_points[g_gx][g_gy+1]);
		}
	}
	glEnd();
	glPopMatrix();
	...
	//glBindTexture(GL_TEXTURE_2D, &g_Text[ure[0]);
	if (!g_scene)
		Texture_Use(&g_Text[2]);
	else if (g_scene==1)
		Texture_Use(&g_Text[12]);
	else
		Texture_Use(&g_Text[14]);
	...
	for( g_gy = 0; g_gy < 64; g_gy++ ) 
		for( g_gx = 0; g_gx < 64; g_gx++) 
			g_points[g_gx][g_gy]=.025*sin(g_speed[g_gx][g_gy]*g_rota+g_phase[g_gx][g_gy]);
	
	glEnable(GL_BLEND);
	glDisable(GL_DEPTH_TEST);
	Texture_Use(&g_Text[10]);
	...
	glLoadIdentity();
	if (!g_scene) 
		glTranslatef(.15,-.11,-1.0f);//MOON
	else if (g_scene==1) 
		glTranslatef(0,-.1075f,-1.0f);//DUSK
	else 
		glTranslatef(0,-.095,-1.0f);//DAY
	if (!g_scene) 
		glScalef(2.0,.07,1);
	else if (g_scene==1) 
		glScalef(1.5,.07,1);
	...
	if ((g_radius>-6.0f)&&(g_radius<3.0f))
	{
		...
		glLoadIdentity();
		if (g_radius<0.0f) 
			glTranslatef(-4.075,.8,-5);
		else 
			glTranslatef(-4.075+g_radius/11,.8,-5);
		glScalef(2,1.25,1);
		glRotatef(-45,0,0,1);
		glTranslatef(2*cos(g_radius),.5*sin(g_radius),-g_radius*1.5);
		...
		for (int i=0; i<g_num+1; i++)
		{
			...
			else
			{
				...
				if ((parts[i].g_a<0.0f)||(parts[i].size<0.0f)) 
					g_rst(i);
			}
			glPopMatrix();
		}
	...
	glEnable(GL_DEPTH_TEST);
	if (!g_scene) 
		g_zeta=66.0f+45.0f*((((GLfloat)g_gettime-limit))/13000.0f)*((((GLfloat)g_gettime-limit))/13000.0f);
	else if (g_scene==1) 
		g_zeta=15.0f+25.0f*(((GLfloat)g_gettime-limit)/1500.0f);
	else if (g_rota<3.0f) 
		g_zeta=15.0f+25.0f*(((GLfloat)g_gettime-limit)/1500.0f);
	else 
		g_zeta=15.0f+25.0f*(((GLfloat)g_gettime-limit)/1500.0f)-30.0f*(1.0f-cos((g_rota-3.0f)/5.0f*3.1415f));
	
	if ((g_scene==2)&&(g_radius<3)) 
		g_radius=-6.0f+(g_zeta-20.0f)*(g_zeta-20.0f)/2000.0f;
	
	glEnable(GL_BLEND);
	glDisable(GL_DEPTH_TEST);
	
	Texture_Use(&g_Text[0]);
	
	glBlendFunc(GL_SRC_ALPHA,GL_ONE);
	glLoadIdentity();
	
	if (!g_scene) 
		glTranslatef(.23,-.06f,-1.0f); // MOON
	else if(g_scene==1) 
		glTranslatef(.07,-.07f,-1.0f); // DUSK
	else 
		glTranslatef(.325,-.08f,-1.0f); // SKY
	...
	glEnable(GL_BLEND);
	glDisable(GL_DEPTH_TEST);
	if ((g_rota<1.0f)|| ((g_rota>9.2f)&&(!g_scene))|| ((g_rota>5.7f)&&(g_scene==1))||
	...
	if (((!g_scene)&&(g_rota>9.45))|| ((g_scene==1)&&(g_rota>5.95)))
		g_scene++;
