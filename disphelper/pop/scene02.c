...
const int size = 64;
float norm[64][64][3];
float a_points[64][64][3];
float camera[3]={-12.8,12.8,5};
float cameraray[3];
float rray[3];
float newcoord[64][64][2];
GLfloat coeff=7.1f;
int a_x, a_y;
float a_float_x, a_float_y, a_float_xb, a_float_yb;
GLfloat	a_xrot;
GLfloat	a_yrot;
GLfloat	a_zrot;
GLfloat a_rad=0.0f;
GLfloat quantos=-1.0f;
GLfloat a_zeta=-1.0f;
GLfloat a_factor=10.0f;
...
GLfloat a_diffuse[]= {0.2f, 0.2f, 0.2f, 1.0f};
GLfloat a_ambient[]= {0.1f, 0.1f, 0.1f, 1.0f};
GLfloat a_specular[]= {.750f, .750f, .750f, 1.0f};
GLfloat a_emission[]= {0.2f, 0.2f, 0.2f, 1.0f};

GLfloat a_shininess = 10.0f;

GLfloat a_LightAmbient[]=		{ 0.5f, 0.5f, 0.5f, 1.0f };
GLfloat a_LightDiffuse[]=		{ 0.5f, 0.5f, 0.5f, 1.0f };
GLfloat a_LightSpecular[]=	{ .5f, .5f, .5f, 1.0f };
GLfloat a_LightPosition[]=	{ 0.0f, 8.0f, -20.0f, 1.0f };
GLfloat a_Sinus[3];
...
void a_InitGL(void)
{
	...
	Texture_Load(&a_Text[1], "data/logoxs.raw");
	Texture_Load(&a_Text[2], "data/white.raw");
	Texture_Load(&a_Text[3], "data/sun2.raw");
	Texture_Load(&a_Text[4], "data/star.raw");
...
bool a_DrawGLScene(GLfloat globtime)
{
	...
	if (a_time>90.0f) 
		a_zeta=-20.0f+10.0f*(1.0f-cos((a_time-90.0f)*3.1415f/10.0f));
	...
	if (a_mod>0.5f) 
		a_mod=1.0-.03f*(a_time-a_gets); 
	else 
		a_mod=.5-0.015f*(a_time-a_gets);
	...
	if (a_zeta>-20.0f) 
		glColor4f(0,0,0,-(a_zeta+20.0f)/40.0f+.25f);
	else 
		glColor4f(0,0,0,.25f);
	...
	glLoadIdentity();
	if (a_time>30.0) 
		glTranslatef(0.0f,1.5f,a_zeta);
	else 
		glTranslatef(0.0f,.5+.5f*(1.0-cos((a_time-2.0f)*3.1415f/28.0f)),a_zeta);
	...
	if (a_time>34.0f) 
		quantos=2.0+(a_time-34.0)/1.5f;

	Texture_Use(&a_Text[1]);
	glMaterialfv(GL_FRONT,GL_DIFFUSE,a_diffuse);
	glMaterialfv(GL_FRONT,GL_AMBIENT,a_ambient);
	glMaterialfv(GL_FRONT,GL_SPECULAR,a_specular);
	glMaterialf(GL_FRONT,GL_SHININESS,10.0f);
	...
	for (int p=0; p<a_num; p++)
	{
		...
		if (a_time<20.0f) 
			glColor4ub(parts[p].r,parts[p].g,parts[p].b,(parts[p].a-(int)((GLfloat)time/8.0f))*(a_time-6.0)/14.0);
		else 
			glColor4ub(parts[p].r,parts[p].g,parts[p].b,parts[p].a-(int)((GLfloat)time/8.0f));

		if (a_time>6.0) 
			a_drawquad(1.125f-.75*p/a_num);
		parts[p].a_mod=parts[p].speed*time/35.0f;
		parts[p].speed=parts[p].speedlim-time/2500000.0f;
		if (parts[p].speed<0.005f) 
			parts[p].speed=0.005f;
