...
void k_InitGL(GLvoid)
{
	glutSetWindowTitle("Scene 12");
	...
	Texture_Load(&k_Text[1], "data/glxcess.raw");
	Texture_Load(&k_Text[2], "data/cl.raw");
	Texture_Load(&k_Text[3], "data/glxcesss.raw");
	Texture_Load(&k_Text[4], "data/crite.raw");
	Texture_Load(&k_Text[5], "data/lightmask.raw");
	Texture_Load(&k_Text[6], "data/cl2.raw");
...
bool k_DrawGLScene(GLfloat globtime)
{
	...
	glLoadIdentity();
	if (k_timer<2.501f) 
		glTranslatef(0,0,-7.0f+3.0f*sin(k_timer*3.1415f*.5f/2.5f));
	else 
	...
	if (k_timer>2.5f)
	{
		...
		Texture_Use(&k_Text[4]);
		
		if (k_timer-2.5f<.75f)
			glColor4f(1,1,1,k_timer-2.5f);
		else
			glColor4f(1,1,1,.75f);
		glScalef(1,-1,1);
		k_drawrect(1,.03125);
