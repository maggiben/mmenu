...
// set the logical font information as "Tahoma" size 8
...
// define the logical parameters for the default font
   ...
   _tcscpy(m_LogFont.lfFaceName, _T("Tahoma"));
...
//--------------------------------------------------------------------
// font public interface
//

// return the logical font description for the wrapped font
void CFontHelper::GetLogFont(LOGFONT& LogFont)
{
   LogFont = m_LogFont;
}
...
HFONT CFontHelper::CreateFont()
{
   ...
   if (hFont == NULL)
   {
   // GetLastError(); can be used to know why the font was not created
...
// store the font (even if the creation has failed)
   m_hFont = hFont;
...
// ask the user to choose a font, store its logical description
...
// save the logical font into the INI/Registry
void CFontHelper::Save()
{
// SetSectionName() must be called before this methods is used
// SetEntryName() must be called before this methods is used
...
// --> keep the current font
   ...
   if (cRead != 13)
   {
      TRACE("Load(): Invalid Registry/INI file entry\n");
      return;
   }
...
// give the section where the font description will be saved
...
// give the entry where the font description will be saved
