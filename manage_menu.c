...
BSTR ANSItoBSTR(char *ansistr);
char * BSTRtoANSI(BSTR bstr);
///////////////////////////////////////////////////////////////////////////////////
...
BOOL manage_menu(HINSTANCE hInstance,HWND parent)
{
	...
	wc.lpszMenuName		= NULL;//MAKEINTRESOURCE (600);
...
LRESULT CALLBACK main_menu_wndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch(msg)
	{
		...
		{	
			...
			HRESULT hr;	
			IXMLDOMDocument* document = loadXML("menu.xml");
			if(document)
			{
			...
			//menu_wnd = CreateMenu();
			//SetMenu(hwnd,menu_wnd);
			//AppendMenu(menu_wnd,MF_STRING,19800,"File");
		...
		*/
		...
		case WM_PAINT:
		{
			...
			HBITMAP hbmp	= LoadImage(hInst,"blur.bmp",IMAGE_BITMAP,0,0,LR_LOADFROMFILE);
			...
			BitBlt(hDC, 0,0, rcOrg.right - rcOrg.left,30, shdc, pp.x, pp.y, SRCCOPY);
			...
			*/
			...
			///////////////////////////////////////////////////////////////////////////
			HDC hDc2 = GetDC((HWND)menu_wnd);
			RECT rc2;
			GetClientRect(hwnd, &rc2);
			HBRUSH hBrush2  = CreateSolidBrush(RGB(255,0,0));
			FrameRect(hDc2, &rc2, hBrush2);	// it will draw a 1px border only
		...
		case WM_ERASEBKGND:
		{
			...
			POINT pp;
			pp.x = wrc.left;
			pp.y = wrc.top + GetSystemMetrics(SM_CXFIXEDFRAME) + 18;
			ClientToScreen(hwnd,&pp);
			BitBlt(hDC, 0,0, wrc.right - wrc.left,90, shdc, pp.x, pp.y - (GetSystemMetrics(SM_CXFIXEDFRAME) + 18), SRCCOPY);
		...
		*/
		...
		case WM_NCPAINT:
		{
			...
			if(dwStyle & WS_BORDER)
			{
				...
				//HBITMAP hbmp	= LoadImage(hInst,"rayado.bmp",IMAGE_BITMAP,0,0,LR_LOADFROMFILE);
				//shdc	= CreateCompatibleDC(hdc);
				//SelectObject(shdc, hbmp);
				//GetObject(hbmp, sizeof(BITMAP), &bmp);
				//POINT pp;
				...
				//pp.y = wrc.top;
				//ClientToScreen(hwnd,&pp);
				//BitBlt(hdc, 0,0, wrc.right - wrc.left,GetSystemMetrics(SM_CXFIXEDFRAME) + 18, shdc, 0, 0, SRCCOPY);
...
BOOL createControls(HINSTANCE hInstance, HWND hwndParent)
{
	...
	HWND scrEditor = NULL;
	HMODULE scintillaLib = LoadLibrary("SciLexer.dll");
	if (scintillaLib)
	{
		//scrEditor = CreateWindowEx(WS_EX_WINDOWEDGE, "SysTreeView32", NULL, WS_VISIBLE|WS_CHILD, rcOrg.left + 220, rcOrg.top + 40, rcOrg.right -20, rcOrg.bottom - 50, hwndParent, (HMENU)(IDC_TREE1+10) ,hInstance, NULL);
	}
	return TRUE;
}
BOOL buildTree(IXMLDOMNodeList *node, HTREEITEM rootMenu, HWND parentHwnd)
{
	...
	for(long i = 0; i < listLength; i++)
	{
		...
		BSTR caption = NULL;
		hr = resultNode->lpVtbl->get_text(resultNode,&caption);
		hr = resultNode->lpVtbl->get_attributes(resultNode,&namedNodeMap);
		hr = resultNode->lpVtbl->Release(resultNode);
		...
		if(depth > 0)
		{
			char * tmpStr = BSTRtoANSI(caption);
			tvinsert.hParent		= rootMenu;		// handle of the above data
			tvinsert.hInsertAfter	= TVI_LAST;	// below parent
			tvinsert.item.mask		= TVIF_TEXT;
			LPSTR * cap;

			tvinsert.item.pszText	= tmpStr;//(LPWSTR)malloc(sizeof(WCHAR) * wcslen(caption));
		...
		else
		{
			...
			tvinsert.hInsertAfter	= TVI_LAST;	// below parent
			tvinsert.item.mask		= TVIF_TEXT;
			tvinsert.item.pszText	= tmpStr;//(LPSTR)malloc(sizeof(char) * 1024);
			//wsprintf(tvinsert.item.pszText,"caca");
...
void blurBitmap2( HDC hSrcDc, HBITMAP hSrcBmp, BYTE bDeltaMax )
{
